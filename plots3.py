...
class PLOTS3:
    ...
    def __init__(self,folder,vmSL,pmSL,fnSL):
...
#        
        ...
        for i,v in enumerate(self.totalplots):
            ...
            for j in range(0,self.numberofdatafile):
                myfilename=self.folderpath+'/'+str(j)+'-'+v['decisionvariables']+'-'+v['optimization']+'-'+v['configuration']+"-vm"+str(self.vmSL).replace(".","p")+"-pm"+str(self.pmSL).replace(".","p")+"-fs"+str(self.fnSL).replace(".","p")+'-selectedSolution.pkl'
                print myfilename
                isthefirst = True
...
#            plt.ylim(ymin=minYaxes[plotId])
            ...
            plt.grid()
            fig.savefig(self.folderpath+'/'+dimensionToPlot+"-vm"+str(self.vmSL)+"-pm"+str(self.pmSL)+"-fs"+str(self.fnSL)+'.pdf')
            plt.close(fig)
